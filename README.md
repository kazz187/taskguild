...
TASKGUILD_PUBLIC_URL="https://taskguild-api.example.com"  # スマホからアクセス可能な URL
```

> **`TASKGUILD_PUBLIC_URL` について**: 通知のアクションボタン（Allow / Deny 等）からの応答時に、Service Worker がこの URL を使ってバックエンドに API コールを行います。未設定の場合 `http://localhost:3100` がデフォルトになるため、スマホからは接続できません。

#### ブラウザでの有効化

1. HTTPS 経由でサイトにアクセス
2. 左サイドバーの **「Enable notifications」** ボタンをタップ
3. ブラウザの通知許可ダイアログで「許可」を選択
4. ボタンが「Notifications on」（シアン色アイコン）に変わることを確認

#### 対応プラットフォーム

| プラットフォーム | アクションボタン | インライン返信 |
|-----------------|-----------------|---------------|
| Chrome Android | 最大 3 個 | 対応 |
| Firefox Android | 最大 2 個 | 非対応 |
| iOS Safari | 非対応（タップで開く） | 非対応 |
| Desktop Chrome | 対応 | 非対応 |
