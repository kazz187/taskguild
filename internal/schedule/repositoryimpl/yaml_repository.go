...
func (r *YAMLRepository) Create(ctx context.Context, s *schedule.Schedule) error {
	r.ensureIndex(ctx)
	...
	if exists {
		return cerr.NewError(cerr.AlreadyExists, "schedule already exists", nil)
	}

	data, err := yaml.Marshal(s)
	if err != nil {
		return cerr.NewError(cerr.Internal, "server error", fmt.Errorf("failed to marshal schedule: %w", err))
	}

	if err := r.storage.Write(ctx, entityPath(s.ProjectID, s.ID), data); err != nil {
		return cerr.WrapStorageWriteError("schedule", err)
	}
...
func (r *YAMLRepository) Get(ctx context.Context, id string) (*schedule.Schedule, error) {
	r.ensureIndex(ctx)
	...
	if !ok {
		return nil, cerr.NewError(cerr.NotFound, "schedule not found", nil)
	}

	data, err := r.storage.Read(ctx, entityPath(pid, id))
	if err != nil {
		return nil, cerr.WrapStorageReadError("schedule", err)
	}

	var s schedule.Schedule
	if err := yaml.Unmarshal(data, &s); err != nil {
		return nil, cerr.NewError(cerr.Internal, "server error", fmt.Errorf("failed to unmarshal schedule: %w", err))
	}

	return &s, nil
}

func (r *YAMLRepository) List(ctx context.Context, projectID string, limit, offset int) ([]*schedule.Schedule, int, error) {
	r.ensureIndex(ctx)
	...
	if projectID != "" {
		...
		if err != nil {
			return nil, 0, cerr.WrapStorageReadError("schedules", err)
		}
...
func (r *YAMLRepository) ListAll(ctx context.Context) ([]*schedule.Schedule, error) {
	all, _, err := r.List(ctx, "", 0, 0)
	if err != nil {
		return nil, err
	}
...
func (r *YAMLRepository) Update(ctx context.Context, s *schedule.Schedule) error {
	r.ensureIndex(ctx)
	...
	if !ok {
		return cerr.NewError(cerr.NotFound, "schedule not found", nil)
	}
...
func (r *YAMLRepository) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return cerr.WrapStorageDeleteError("schedule", err)
	}
